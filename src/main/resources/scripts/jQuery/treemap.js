...
$.fn.treemap = function(w,h,options) {
	options = $.extend({labelCell:0,dataCell:1,colorCell:2,headHeight:20,borderWidth:1,borderColor:"#000",getColor:treemap.getColor,sort:true,nested:false,legend:false},options);
...
var treemap = {
	...
	layoutRow : function(row,w,orientation,s,options,last) {
		...
		var rl = row.length-1,sum = 0, bw = options.borderWidth, bc = options.borderColor, bw2 = bw*2, cells = []; 
		for(var i=0;i<=rl;i++) {
			...
			if(bw && rowDim>bw2 && varDim>bw2) {
				...
				cellStyles.border = bw+'px solid ' + bc;
			...
			if(hier) {
				...
				if(n[2]) cellStyles.backgroundColor = options.getColor(n[2],options);
			}
			if (n[3]) {
				cell.attr({"id": n[3]});
			}
